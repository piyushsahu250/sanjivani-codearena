...
function generateCertificatePdf({ studentName, courseName, certificateCode, issuedAt }, res) {
  ...
  doc.font("Helvetica").fontSize(13).fillColor("#6B6A5F").text("This is to certify that", 0, 200, { align: "center" });
  doc.font("Helvetica-Bold").fontSize(26).fillColor("#4F9D6E").text(studentName, 0, 225, { align: "center" });
  doc.font("Helvetica").fontSize(13).fillColor("#6B6A5F")
    .text(`has successfully completed the`, 0, 270, { align: "center" });
  doc.font("Helvetica-Bold").fontSize(20).fillColor("#1C1B18").text(`${courseName} Learning Course`, 0, 292, { align: "center" });

  doc.font("Helvetica").fontSize(11).fillColor("#6B6A5F")
    .text(`Completion Date: ${new Date(issuedAt).toLocaleDateString()}`, 0, 350, { align: "center" });
  doc.font("Helvetica").fontSize(11).fillColor("#6B6A5F")
    .text(`Certificate ID: ${certificateCode}`, 0, 368, { align: "center" });
