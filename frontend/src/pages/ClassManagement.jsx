...
export default function ClassManagement() {
  ...
  const [form, setForm] = useState({ name: "", code: "", instituteId: "" });
  const [error, setError] = useState("");
  const [saving, setSaving] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [editForm, setEditForm] = useState({ name: "", code: "", instituteId: "" });
  ...
  async function handleCreate(e) {
    ...
    try {
      ...
      setForm({ name: "", code: "", instituteId: form.instituteId });
      load(filterInstituteId);
    } catch (err) {
  ...
  function startEdit(cls) {
    setEditingId(cls.id);
    setEditForm({ name: cls.name, code: cls.code || "", instituteId: cls.instituteId || "" });
  }
                ...
                <>
                  <div>
                    ...
                    <div className="mono" style={{ fontSize: 12, color: "var(--ink-dim)", marginTop: 4 }}>
                      {cls.institute?.name || "No institute"}
                    </div>
                  </div>
