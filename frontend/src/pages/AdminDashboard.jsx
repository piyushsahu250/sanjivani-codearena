    ...
    <div>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "48px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            <h1>Faculty control room</h1>
            <ChalkUnderline />
          </div>
          <div style={{ display: "flex", gap: 10 }}>
            <Link to="/admin/questions/new" className="btn btn-ghost">+ New question</Link>
            <Link to="/admin/tests/new" className="btn btn-primary">+ New test</Link>
          </div>
        </div>
          ...
          {tests.map((test) => (
            <div key={test.id} className="card" style={{ padding: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <Link to={`/admin/tests/${test.id}/results`} className="btn btn-ghost">Results</Link>
